...
```{r}
task_b <- function(main, sub, method = method_, maxit = maxit_, m = m_, pool.syn = F){
  main["X"] <- NA
...
```{r}
task_c <- function(main, sub, method = method_, maxit = 10, m = 25){
  main["X"] <- NA
  sub["Y"] <- NA
  
  c_data <- rbind(main, sub)
  
  c_imp <- mice(c_data, method = method_, maxit = maxit, m = m, printFlag = F)
...
bias_values_b0_pmm <- bias_values_b1_pmm <-
  bias_values_b0_norm <- bias_values_b1_norm <- 
  bias_values_b0_midastouch <- bias_values_b1_midastouch <-
  bias_values_b0_cart <- bias_values_b1_cart <- 
  setNames(
    data.frame(matrix(ncol = length(names)-1, nrow = n)),
    names[2:length(names)])

bias_values_b0_pmm$method <- bias_values_b1_pmm$method <- "pmm"
bias_values_b0_norm$method <- bias_values_b1_norm$method <- "norm"
bias_values_b0_midastouch$method <- bias_values_b1_midastouch$method <- "midastouch"
bias_values_b0_cart$method <- bias_values_b1_cart$method <- "cart"
...
beta_zeros_long <- beta_ones_long <- setNames(data.frame(matrix(ncol=3, nrow=0)), c("method", "task", "bias"))

for (i in names(beta_zeros)){
  beta_zeros_long <- rbind(beta_zeros_long, pivot_longer(beta_zeros[[i]], -method, names_to = "task", values_to = "bias"))
  beta_ones_long <- rbind(beta_ones_long, pivot_longer(beta_ones[[i]], -method, names_to = "task", values_to = "bias"))
}

beta_zeros_long$beta <- "beta0"
beta_ones_long$beta <- "beta1"

bias_data <- rbind(beta_zeros_long, beta_ones_long)

ggplot(bias_data, aes(x = task, y = bias)) + 
  geom_boxplot() + 
  facet_wrap(~method + beta, ncol = 2) +
  theme_minimal() + 
  ggtitle("Bias")
```
